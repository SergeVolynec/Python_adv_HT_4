...
def menu(balance: Decimal, count: int, is_flag: bool, history: list):
    ...
    if choice == '3':
        print(balance)
        print("История счета: ", *history)
        is_flag = False
        return balance, is_flag
    elif choice == '1':
        balance = give_money(balance, history)
        count += 1
    elif choice == '2':
        balance = get_money(balance, history)
        count += 1
    else:
...
def get_money(balance: Decimal, history: list):
    ...
    if money_to_get % MIN_SUM == 0:
        ...
        if money_to_get + procent <= balance:
            history.append(-(money_to_get + procent))
...
if __name__ == '__main__':
    ...
    history = []
    while is_flag:
        balance, is_flag = menu(balance, count, is_flag, history)
